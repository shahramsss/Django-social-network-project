...
from .forms import RegisterAccountForm, LoginAccountForm ,  EditUserForm
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            user = User.objects.create_user(
                cd['username'], cd['email'], cd['password_2'])
            messages.success(
                request, 'you registered successfully', extra_tags='success')
            return redirect('home:home')
        return render(request, self.template_name, {'form': form})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                messages.success(
                    request, 'you loged in successfully', 'success')
                if self.next:
                    return redirect(self.next)
                return redirect('home:home')
            messages.error(
                request, 'username and password is wrong', 'warning')
        return render(request, self.template_name, {'form': form})
...
class UsersPosts(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        if relation.exists():
            is_following = True
        return render(request, 'account/user_posts.html', {'posts': posts, 'user': user, 'is_following': is_following})
...
class UserFollow(LoginRequiredMixin, View):
    def get(self, request, user_id):
        ...
        if relation.exists():
            messages.error(
                request, 'you are allready following this user', 'danger')
        else:
            Relation(from_user=request.user, to_user=user).save()
            messages.success(request, 'you follow this user', 'success')
        return redirect('account:user_posts', user.id)
...
class EditUserView(LoginRequiredMixin , View):
    ...
    def post(self, request):
        ...
        return redirect('account:profile_account' , )